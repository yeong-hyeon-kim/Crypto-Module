...
namespace CryptoModule
{
    public class AES
    {
        public string AESEncrypt256(string Value, string Password)
        {
            string EncryptedData = "";

            try
            {
                // Rijndael Class 인스턴스 생성
                RijndaelManaged RijndaelCipher = new RijndaelManaged();

                // 입력받은 문자열을 바이트 배열로 변환
                byte[] PlainText = Encoding.Unicode.GetBytes(Value);

                // 딕셔너리 공격을 대비해서 키를 더 풀기 어렵게 만들기 위해서 Salt를 사용
                // 원본 메세지에 문자열을 추가하여 다이제스트를 생성하는 것을 솔팅(Salting) 이라고 한다.
                ...
                // CryptoStream 객체를 암호화된 데이터를 쓰기 위한 용도로 선언
        ...
        public string AESDecrypt256(string Value, string Password)
        {
